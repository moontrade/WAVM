...
void EmitFunctionContext::local_get(GetOrSetVariableImm<false> imm)
{
	...
	push(irBuilder.CreateLoad(localPointers[imm.variableIndex]->getType()->getPointerElementType(),
...
void EmitFunctionContext::global_get(GetOrSetVariableImm<true> imm)
{
	...
	if(globalType.isMutable)
	{
		...
		auto ptr = irBuilder.CreateLoad(contextPointerVariable->getType()->getPointerElementType(),
										contextPointerVariable);
		llvm::Value* globalPointer = irBuilder.CreateInBoundsGEP(
			ptr->getType()->getScalarType()->getPointerElementType(), ptr, globalDataOffset);
